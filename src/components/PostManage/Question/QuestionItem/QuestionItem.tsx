...
import { FollowQuestion, UnfollowQuestion, CheckFollowStatus } from '../../../../services/FollowServices';
...
interface MutationContext {
    ...
    previousQuestions: Question[] | undefined;
}
...
const QuestionItem: React.FC<QuestionItemProps> = ({ question, onDelete }) => {
    ...
    const followMutation = useMutation<void, Error, number, MutationContext>({
        ...
        onMutate: async (questionId: number) => {
            await queryClient.cancelQueries({ queryKey: ['followStatus', questionId] });
            await queryClient.cancelQueries({ queryKey: ['questions'] });

            const previousFollowStatus = queryClient.getQueryData<FollowStatus>(['followStatus', questionId]);
            const previousQuestions = queryClient.getQueryData<Question[]>(['questions']);

            // Optimistic update cho follow status
            queryClient.setQueryData<FollowStatus>(['followStatus', questionId], { isFollowing: true });
        ...
        onError: (error, _questionId, context) => {
            if (context) {
                queryClient.setQueryData(['followStatus', question.id], context.previousFollowStatus);
                queryClient.setQueryData(['questions'], context.previousQuestions);
            }
            toast.error(error.message || 'Failed to follow question');
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['followStatus', question.id] });
    ...
    const unfollowMutation = useMutation<void, Error, number, MutationContext>({
        ...
        onMutate: async (questionId: number) => {
            ...
            queryClient.setQueryData<FollowStatus>(['followStatus', questionId], { isFollowing: false });
        ...
        >
            <div className='border-t border-content3 p-4 relative '>
                <div className='flex items-center justify-between'>
                    <Link to={"/"} className='text-md font-bold hover:underline'>{question.title}</Link>
                    <Button
                        isIconOnly
                ...
                <div className='flex items-center gap-x-1 opacity-80 text-xs flex-wrap !items-center mt-1 '>
                    <button className='font-bold hover:underline'>{question.answerCount} answers</button>
                    <GoDotFill className="w-1 h-1 hidden sm:block" />
                    <span>{question.lastFollowed ? `Last followed ${format(date)}` : `No one followed`}</span>
                </div>
                <div className='flex items-center justify-between mt-3'>
                    <div className='flex items-center gap-x-1 '>
                        ...
                        <Button
                            ...
                            isLoading={followMutation.isPending || unfollowMutation.isPending || isCheckingFollow}
                        >
                    ...
                    <Popover placement="top-start">
                        <PopoverTrigger>
                            <Button size="sm" variant="flat" radius="full" className="bg-transparent" isIconOnly>
                                <HiOutlineDotsHorizontal className="text-lg cursor-pointer" />
                            </Button>
                        </PopoverTrigger>
                        <PopoverContent className="p-0 rounded-sm bg-content1 w-min">
                            <div className="">
                                <Button className='hover:bg-content3 bg-transparent text-md font-light w-full !justify-start text-xs font-semibold' size="sm" radius='none'>
                ...
                <AnswerModal isOpen={isOpen} onOpenChange={onOpenChange} question={question} />
            </div>
        </motion.div>
    );
};
