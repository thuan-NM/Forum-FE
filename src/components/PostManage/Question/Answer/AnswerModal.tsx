import { Modal, ModalContent, ModalBody, ModalFooter, Button, User } from "@heroui/react";
...
const AnswerModal: React.FC<AnswerModalProps> = ({ isOpen, onOpenChange, question }) => {
    ...
    const mutation = useMutation<void, Error, AnswerFormData, MutationContext>({
        mutationFn: CreateAnswer,
        onMutate: async (data: AnswerFormData) => {
            ...
            queryClient.setQueryData<Question[]>(['questions'], (old) =>
                old?.map((q) =>
                    q.id === data.question_id ? { ...q, answerCount: q.answerCount + 1 } : q
                )
            );
                                        ...
                                        {isVisible && editor ? (
                                            ...
                                            >
                                                <MenuBar
                                                    ...
                                                    setShowEmojiPicker={() => setShowEmojiPicker(!showEmojiPicker)}
                                                />
                                            </motion.div>
                                        ) : null}
                                    </AnimatePresence>
